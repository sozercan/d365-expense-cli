...
// DynamicsBootstrapState contains only the current Expense workspace control
// identifiers discovered from the rendered page. Authentication and protocol
// state continue to come exclusively from captured network traffic.
type DynamicsBootstrapState struct {
...
type Options struct {
	...
	DynamicsBootstrap      *DynamicsBootstrapState
	ExistingTargetID       string
	PreserveExistingTarget bool
}
...
func Capture(parent context.Context, options Options) (*har.Archive, error) {
	...
	preserveTarget := strings.TrimSpace(options.ExistingTargetID) != "" && options.PreserveExistingTarget
	if preserveTarget {
		go func() {
			<-captureCtx.Done()
			cancelCapture()
	...
	if strings.TrimSpace(options.ExistingTargetID) != "" {
		tabCtx, cancelTab = chromedp.NewContext(allocatorCtx, chromedp.WithTargetID(cdptarget.ID(options.ExistingTargetID)))
	} else {
	...
	inspectDynamicsBootstrap := chromedp.ActionFunc(func(ctx context.Context) error {
		if options.DynamicsBootstrap == nil {
			return nil
		}
		var discovered struct {
			WorkspaceRootID string `json:"workspaceRootId"`
			NewReportID     string `json:"newReportId"`
		}
		expression := `(() => {
		  const matches = [...document.querySelectorAll('[data-dyn-controlname="NewExpenseReportReportsTab"]')];
		  if (matches.length !== 1) return {workspaceRootId:'', newReportId:''};
		  return {
		    workspaceRootId: matches[0].getAttribute('data-dyn-rootserverid') || '',
		    newReportId: matches[0].getAttribute('data-dyn-serverid') || ''
		  };
		})()`
		if err := chromedp.Evaluate(expression, &discovered).Do(ctx); err != nil {
			return fmt.Errorf("discover Dynamics workspace controls: %w", err)
		}
		if strings.TrimSpace(discovered.WorkspaceRootID) == "" || strings.TrimSpace(discovered.NewReportID) == "" {
			return errors.New("discover Dynamics workspace controls: New expense report control not found")
		}
		options.DynamicsBootstrap.WorkspaceRootID = discovered.WorkspaceRootID
		options.DynamicsBootstrap.NewReportID = discovered.NewReportID
		return nil
	})
...
func (options Options) withDefaults() Options {
	...
	if strings.TrimSpace(options.CreatorName) == "" {
		options.CreatorName = "d365-expense-cli"
	}
